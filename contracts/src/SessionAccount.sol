...
import { TokenCallbackHandler } from "./4337/callback/TokenCallbackHandler.sol";
import { ITokenEntryPoint } from "./4337/interfaces/ITokenEntryPoint.sol";
import { IOwnable } from "./4337/interfaces/IOwnable.sol";
	...
	UUPSUpgradeable
{
	 ...
	 * a new implementation of SessionAccount must be deployed with the new EntryPoint address, then upgrading
	 * the implementation by calling `upgradeTo()`
	 */
	function initialize(address sessionAdmin, address anOwner) public virtual initializer {
		__Ownable_init(anOwner);
		__UUPSUpgradeable_init();

		_initialize(sessionAdmin, anOwner);
	}

	function _initialize(address sessionAdmin, address anOwner) internal virtual {
	...
	function _requireFromEntryPointOrOwnerOrTokenEntryPoint() internal view {
		require(
			msg.sender == address(entryPoint()) || msg.sender == owner() || msg.sender == tokenEntryPoint(),
	...
	function _validateSignature(
		...
		address sender = hash.recover(userOp.signature);
		if (owner() != sender && !hasValidSession(sender)) return SIG_VALIDATION_FAILED;
		return 0;
	}
	...
	function isValidSignature(bytes32 _hash, bytes calldata _signature) external view override returns (bytes4) {
		...
		if (signer == owner()) {
			return 0x1626ba7e;
		} else if (hasValidSession(signer)) {
			return 0x1626ba7e;
		} else {
			return 0xffffffff;
		}
	...
	function recoverOwnership(address newOwner) public virtual {
		...
		require(msg.sender == ownable.owner(), "Ownable: not TokenEntryPoint owner");
	...
	// related to sessions
	address private _sessionAdmin;
	mapping(address => uint48) private _session;

	error SessionAdminOrAccountOwnerOnly(address account);

	function _checkSessionAdminOrAccountOwner() internal view virtual {
		if (msg.sender != _sessionAdmin && msg.sender != owner() && msg.sender != address(this)) {
			revert SessionAdminOrAccountOwnerOnly(msg.sender);
		}
	}

	modifier onlySessionAdmin() {
		_checkSessionAdminOrAccountOwner();
		_;
	}

	function setSessionAdmin(address newSessionAdmin) public onlyAccountOwner {
		_sessionAdmin = newSessionAdmin;
	}

	function startSession(address sessionAddress, uint48 duration) public onlySessionAdmin {
		_session[sessionAddress] = uint48(block.timestamp + duration);
	}

	function endSession(address sessionAddress) public onlyAccountOwner {
		_session[sessionAddress] = 0;
	}

	function hasValidSession(address sessionAddress) public view returns (bool) {
		return _session[sessionAddress] > block.timestamp;
	}
	...
	function _authorizeUpgrade(address newImplementation) internal view override onlyAccountOwner {
		(newImplementation);
	}
}
